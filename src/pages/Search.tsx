...
export function Search() {
  ...
  const filteredRecipes = useMemo(() => {
    return recipes.filter(recipe => {
      ...
      const matchIngredient = filters.ingredient === '' || recipe.ingredients.some(i => i.name.toLowerCase().includes(filters.ingredient.toLowerCase()));

      return matchQuery && matchTime && matchDifficulty && matchCategory && matchRegion && matchTag && matchIngredient;
    });
  }, [query, filters]);
    ...
    <div className="pb-6 pt-6 px-4 max-w-md md:max-w-2xl lg:max-w-4xl mx-auto min-h-full bg-stone-50">
      ...
      {activeFiltersCount > 0 && (
        <div className="flex flex-wrap gap-2 mb-6">
          ...
          <button
            onClick={() => setFilters({ time: '', difficulty: '', category: '', region: '', diet: 'Halal', tag: '', ingredient: '' })}
      ...
      {showFilters && (
        <div className="fixed inset-0 z-[60] flex items-end bg-stone-900/40 backdrop-blur-sm transition-opacity">
          <div className="bg-white w-full max-w-md md:max-w-2xl lg:max-w-4xl mx-auto rounded-t-3xl p-6 pb-safe max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              <div>
                <h3 className="font-semibold text-stone-900 mb-3">Bahan yang Dimiliki</h3>
                <Input
                  value={filters.ingredient}
                  onChange={(e) => setFilters(prev => ({ ...prev, ingredient: e.target.value }))}
                  placeholder="Misal: ayam, tempe, santan..."
                  className="bg-white shadow-sm border-stone-200 rounded-xl"
                />
              </div>
            ...
            <div className="mt-8 pt-4 border-t border-stone-100 flex gap-4">
              <Button
                ...
                onClick={() => {
                  setFilters({ time: '', difficulty: '', category: '', region: '', diet: 'Halal', tag: '', ingredient: '' });
                }}
              >
                Reset
              </Button>
