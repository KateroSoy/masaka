          ...
          <div className="mt-auto flex items-center justify-between text-xs text-stone-500 font-medium">
            ...
            <div className="flex items-center gap-1" title={recipe.difficulty}>
              <div className="flex gap-0.5">
                <div className={`w-2 h-2 rounded-full ${recipe.difficulty === 'Mudah' || recipe.difficulty === 'Sedang' || recipe.difficulty === 'Sulit' ? 'bg-green-500' : 'bg-stone-200'}`} />
                <div className={`w-2 h-2 rounded-full ${recipe.difficulty === 'Sedang' || recipe.difficulty === 'Sulit' ? 'bg-yellow-500' : 'bg-stone-200'}`} />
                <div className={`w-2 h-2 rounded-full ${recipe.difficulty === 'Sulit' ? 'bg-red-500' : 'bg-stone-200'}`} />
              </div>
            </div>
          </div>
        </div>
